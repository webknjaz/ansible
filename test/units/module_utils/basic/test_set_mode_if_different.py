...
@pytest.fixture
def am_check_mode(ansible_module):
...
def test_no_mode_given_returns_previous_changes(ansible_module, mock_stats, mock_lchmod, mocker, previous_changes, check_mode, exists):
    ...
    assert ansible_module.set_mode_if_different('/path/to/file', None, previous_changes) == previous_changes
...
@pytest.mark.parametrize('mode, check_mode, ansible_module_args',
                         ...
                         indirect=['ansible_module_args'])
def test_mode_changed_to_0660(ansible_module, mock_stats, mocker, mode, check_mode):
    ...
    assert ansible_module.set_mode_if_different('/path/to/file', mode, False)
    if check_mode:
        assert not m_lchmod.called
    else:
...
@pytest.mark.parametrize('mode, check_mode, ansible_module_args',
                         product(SYNONYMS_0660, (True, False), ({},)),
                         indirect=['ansible_module_args'])
def test_mode_unchanged_when_already_0660(ansible_module, mock_stats, mocker, mode, check_mode):
    # Note: This is for checking that all the different ways of specifying
    # 0660 mode work.  It cannot be used to check that setting a mode that is
    # not equivalent to 0660 works.
    ansible_module.check_mode = check_mode
    ...
    assert not ansible_module.set_mode_if_different('/path/to/file', mode, False)
    assert not m_lchmod.called
...
@pytest.mark.parametrize('check_mode, ansible_module_args',
                         product((True, False), ({},)),
                         indirect=['ansible_module_args'])
def test_missing_lchmod_is_not_link(ansible_module, mock_stats, mocker, monkeypatch, check_mode):
    ...
    assert ansible_module.set_mode_if_different('/path/to/file/no_lchmod', 0o660, False)
    if check_mode:
...
@pytest.mark.parametrize('check_mode, ansible_module_args',
                         product((True, False), ({},)),
                         indirect=['ansible_module_args'])
def test_missing_lchmod_is_link(ansible_module, mock_stats, mocker, monkeypatch, check_mode):
    """Some platforms have lchmod (*BSD) others do not (Linux)"""
